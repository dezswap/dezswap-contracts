...
fn supply_querier() {
    ...
    assert_eq!(
        query_token_info(&deps.as_ref().querier, deps.api.addr_make("liquidity0000"))
            .unwrap()
            .total_supply,
...
fn test_asset() {
    ...
    assert_eq!(
        token_asset
            .clone()
            .into_msg(deps.api.addr_make("addr0000"))
            .unwrap(),
    ...
    assert_eq!(
        token_asset
            .into_submsg(deps.api.addr_make("addr0000"))
            .unwrap(),
...
#[test]
fn query_dezswap_pair_contract() {
    let mut deps = mock_dependencies(&[]);
