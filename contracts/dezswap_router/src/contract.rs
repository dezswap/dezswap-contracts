...
use dezswap::pair::SimulationResponse;
use dezswap::querier::{query_pair_info, reverse_simulate, simulate};
use dezswap::router::{
...
const CONTRACT_NAME: &str = "crates.io:dezswap-router";
...
pub fn instantiate(
    ...
    CONFIG.save(
        deps.storage,
        &Config {
            dezswap_factory: deps.api.addr_canonicalize(&msg.dezswap_factory)?,
        },
    )?;
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        dezswap_factory: deps.api.addr_humanize(&state.dezswap_factory)?.to_string(),
    };
...
fn simulate_swap_operations(
    ...
    let dezswap_factory = deps.api.addr_humanize(&config.dezswap_factory)?;
