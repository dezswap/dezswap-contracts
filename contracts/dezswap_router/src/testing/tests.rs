...
use dezswap::pair::ExecuteMsg as PairExecuteMsg;
use dezswap::router::{
...
fn proper_initialization() {
    ...
    let msg = InstantiateMsg {
        dezswap_factory: deps.api.addr_make("dezswapfactory").to_string(),
    };
        ...
        vec![
            SubMsg::new(CosmosMsg::Wasm(WasmMsg::Execute {
                ...
                msg: to_json_binary(&ExecuteMsg::ExecuteSwapOperation {
                    operation: SwapOperation::DezSwap {
                        offer_asset_info: AssetInfo::NativeToken {
                            denom: "ukrw".to_string(),
                        },
                        ask_asset_info: AssetInfo::Token {
...
fn execute_swap_operation() {
    ...
    let info = message_info(&Addr::unchecked(MOCK_CONTRACT_ADDR), &[]);
    let res = execute(deps.as_mut(), mock_env(), info, msg).unwrap();
    assert_eq!(
        res.messages,
        vec![SubMsg::new(
...
fn query_reverse_routes_with_from_native() {
    ...
    let msg = QueryMsg::ReverseSimulateSwapOperations {
        ...
        operations: vec![SwapOperation::DezSwap {
            offer_asset_info: AssetInfo::NativeToken {
                denom: "ukrw".to_string(),
            },
            ask_asset_info: AssetInfo::Token {
